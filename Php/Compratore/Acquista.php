...
if($_COOKIE['tipo_utente']==3)
{
    ...
    $queryvis = mysql_query("SELECT * FROM carrello INNER JOIN auto ON carrello.id = auto.id WHERE carrello.incar ='".$_COOKIE["utente"]."'") or die("query non riuscita".mysql_error());
    ...
    while($row = mysql_fetch_object($queryvis))
    {
        ...
        $queryaut = "UPDATE auto SET compratore = '".$_COOKIE["utente"]."' WHERE '".$row->compratore ."' = '".$_COOKIE["utente"]."'";

        $querycar = "DELETE FROM carrello WHERE '".$row->carrello.id ."' = '".$row->auto.id ."' AND incar = '".$_COOKIE["utente"]."'";
